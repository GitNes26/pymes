...
// Comisión de LA PLATAFORMA (no la de Mercado Pago) sobre una venta ya cobrada: solo para las
// tiendas del plan gratis (o los planes en feePlans) y solo si tienen la cuenta conectada por
// OAuth (mp_connected) — eso se valida en server.js, aquí solo se calcula el monto. La absorbe
// el negocio: Mercado Pago la separa del pago antes de pagarle, el cliente no ve ningún cargo
// distinto por esto.
function platformFee(total, biz, feePercent, feePlans) {
  const pct = Math.max(0, Math.min(30, Number(feePercent) || 0));
  if (pct <= 0) return 0;
  const plans = Array.isArray(feePlans) && feePlans.length ? feePlans : ['free'];
  if (!biz || !plans.includes(biz.plan || 'free')) return 0; // sin tienda no se puede confirmar el plan: no se cobra
  return Math.round((Number(total) || 0) * pct) / 100;
}

module.exports = { priceMarkupFactor, markupAmount, applyMarkup, mpCardFee, platformFee, parseCustomTags, customTagsJson, parseSpecs, orderLines };
